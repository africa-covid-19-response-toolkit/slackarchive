...
class Messages extends Model {
	...
	public static function ESsearchMessages( $searchterm, $channel ) {
		...
		$searchresults = Messages::complexSearch( [
			'body' => [
				...
				"from"      => ( $page > 1 ) ? ( $page - 1 ) * config( 'slackarchive.num_posts_per_page', 100 ) : 0,
				"size"      => config( 'slackarchive.num_posts_per_page', 10 ),
			]
		] )->paginate( config( 'slackarchive.num_posts_per_page', 100 ) );
	...
	public function getMessages( $channelid ) {
		...
		return Cache::tags( [ 'messages_cache' ] )->remember( 'app_messages_' . $channelid . '_order_by_ts_page_' . $page,
			config( 'slackarchive.query_cache.timeout_short' ), function () use ( $channelid ) {
