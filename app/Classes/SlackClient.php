...
class SlackClient {
	...
	public function updateUsers() {
		...
		foreach ( $members as $member ) {
			...
			$user->email   = $member->profile->email ?? null;
	...
	public function updateMessages( $limit = 300 ) {
		...
		foreach ( $channellist as $c ) {
			...
			if ( ! isset( $channelmessages->messages ) ) {
				continue;
			}
			...
			foreach ( $channelmessages->messages as $message ) {
				...
				if ( $m->wasRecentlyCreated ) {
					...
					if ( isset( $message->files ) && ! empty( $message->files[0]->name ) ) {

						if ( ! empty( $message->files[0]->url_private ) ) {
							//$file_url    = slack_file_downloader( $message->files[0]->url_private );
							$m->file_url = $message->files[0]->url_private;
						}
